...
typedef struct
{
    ...
    uint32_t total;
    uint32_t last;
    uint8_t    payload_buffer[64 + SHA1_HASH_SIZE + HWCRYPTO_ALIGNMENT_BYTES - 1];/*[64 + 32 + HWCRYPTO_ALIGNMENT_BYTES -1];*/
    uint8_t left_over_bytes[64];
    sha1_context_t hw_ctx;
}mbedtls_sha1_hw_context;

typedef struct
{
    mbedtls_sha1_hw_context ctx;
...
typedef struct
{
    ...
    uint32_t total;
    uint32_t last;
    uint8_t    payload_buffer[64 + SHA256_HASH_SIZE + HWCRYPTO_ALIGNMENT_BYTES - 1];/*[64 + 32 + HWCRYPTO_ALIGNMENT_BYTES -1];*/
    uint8_t left_over_bytes[64];
    sha256_context_t hw_ctx;
} mbedtls_sha256_hw_context;

typedef struct
{
    mbedtls_sha256_hw_context ctx;
...
typedef struct
{
    ...
    uint32_t last;
    uint8_t left_over_bytes[64];
    uint8_t    payload_buffer[64 + MD5_HASH_SIZE + HWCRYPTO_ALIGNMENT_BYTES - 1];/*[64 + 32 + HWCRYPTO_ALIGNMENT_BYTES -1];*/
    md5_context_t hw_ctx;
} mbedtls_md5_hw_context;

typedef struct
{
    mbedtls_md5_hw_context ctx;
} md5_hmac_hw_context;
 ...
 * @param[in]   is224           0 : SHA256 1: SHA224
 */
platform_result_t platform_hwcrypto_sha2_hmac( const unsigned char* key, uint32_t keylen, const unsigned char* input, uint32_t ilen, unsigned char output[ SHA256_HASH_SIZE ], int32_t is224 );
 ...
 * @param[out]  output          buffer to hold result of authentication
 */
platform_result_t platform_hwcrypto_sha1( const unsigned char* input, uint32_t ilen, unsigned char hash_output[ SHA1_HASH_SIZE ] );
 ...
 * @param[out]  output          buffer to hold result of authentication
 */
platform_result_t platform_hwcrypto_sha1_hmac( const unsigned char* key, uint32_t keylen, const unsigned char* input, uint32_t ilen, unsigned char output[ SHA1_HASH_SIZE ] );
 ...
 * @param[out]  output          buffer to hold result of authentication
 */
platform_result_t platform_hwcrypto_md5( const unsigned char* input, uint32_t ilen, unsigned char hash_output[ MD5_HASH_SIZE ] );
 ...
 * @param[out]  output          buffer to hold result of authentication
 */
platform_result_t platform_hwcrypto_md5_hmac( const unsigned char* key, uint32_t keylen, const unsigned char* input, uint32_t ilen, unsigned char output[ MD5_HASH_SIZE ] );
