...
{
    platform_result_t   result;
    uint32_t length = ilen < 64 ? ( ilen + 64 ) : ilen ;
    ...
    else
    {
        ...
        unsigned char* aligned_outbuf;

        crypto_cmd_t cmd =
        { 0 };

        /* populate_input_descriptors function pads 10* to input buffer to make it multiples of 512 bits.
           After padding, input size given to hardware is nearest multiple of 64 from input size.
           So output buffer size also should be "nearest multiple of 64 from input size" + "hash output size". */
        outbuf = (unsigned char *)calloc( length + SHA256_HASH_SIZE + SHA_PADDING_MAX + HWCRYPTO_ALIGNMENT_BYTES - 1, 1 );
        if( NULL == outbuf)
        {
            return PLATFORM_ERROR;
        }
        aligned_outbuf =  (unsigned char*) ROUND_UP( ( unsigned long ) outbuf, HWCRYPTO_ALIGNMENT_BYTES );

        cmd.source                          = ( unsigned char* )input;
        cmd.output                          = ( unsigned char* )aligned_outbuf;
        ...
        unsigned char output[ SHA256_HASH_SIZE ], int32_t is224)
{
    ...
    else
    {
        ...
        outbuf = (unsigned char *)calloc( length + SHA256_HASH_SIZE + HWCRYPTO_ALIGNMENT_BYTES - 1, 1 );
        if( NULL == outbuf)
        {
            return PLATFORM_ERROR;
        }
...
platform_result_t platform_hwcrypto_sha1_hmac_finish( sha1_context_t* ctx, uint8_t* input, int32_t ilen, unsigned char output[ SHA1_HASH_SIZE ] )
{
    ...
    platform_hwcrypto_sha1( store_buffer, ( HMAC_OUTERHASHCONTEXT_SIZE + sha1_output_size ), output );
 ...
 * @param[out]  output          buffer to hold result of authentication
 */
platform_result_t platform_hwcrypto_sha1(const unsigned char* input, uint32_t ilen, unsigned char output[ SHA1_HASH_SIZE ] )
{
    platform_result_t   result;
        ...
        unsigned char output[ SHA1_HASH_SIZE ] )
{
    ...
    else
    {
        ...
        outbuf = (unsigned char *)calloc( length + SHA1_HASH_SIZE + HWCRYPTO_ALIGNMENT_BYTES - 1, 1 );
        if( NULL == outbuf)
        {
            return PLATFORM_ERROR;
        }
...
platform_result_t platform_hwcrypto_md5_hmac_finish( md5_context_t* ctx, uint8_t* input, int32_t ilen, unsigned char output[ MD5_HASH_SIZE ] )
{
    ...
    platform_hwcrypto_md5( store_buffer, ( HMAC_OUTERHASHCONTEXT_SIZE + md5_output_size ), output );
 ...
 * @param[out]  output          buffer to hold result of authentication
 */
platform_result_t platform_hwcrypto_md5( const unsigned char* input, uint32_t ilen, unsigned char output[ MD5_HASH_SIZE ] )
{
    platform_result_t   result;
    ...
    else
    {
        ...
        /* populate_input_descriptors function pads 10* to input buffer to make it multiples of 512 bits.
           After padding, input size given to hardware is nearest multiple of 64 from input size.
           So output buffer size also should be "nearest multiple of 64 from input size" + "hash output size". */
        outbuf = (unsigned char *)calloc( length + SHA256_HASH_SIZE + MD5_PADDING_MAX + HWCRYPTO_ALIGNMENT_BYTES - 1, 1 );
        if( NULL == outbuf)
        {
            return PLATFORM_ERROR;
        }
        ...
        unsigned char output[ MD5_HASH_SIZE ] )
{
    ...
    else
    {
        ...
        outbuf = (unsigned char *)calloc( length + MD5_HASH_SIZE + HWCRYPTO_ALIGNMENT_BYTES - 1, 1 );
        if( NULL == outbuf)
        {
            return PLATFORM_ERROR;
        }
