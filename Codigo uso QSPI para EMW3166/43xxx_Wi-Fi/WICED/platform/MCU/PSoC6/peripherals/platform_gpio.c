 ...
 ******************************************************/
 /*
  * Internal data structure describing the PSoC6 possiblechip variations for each Pin
  * for its expected use as GPIO. The other HSIOM modes and implications are Not addressed here currently
  */

typedef struct
{
    wiced_bool_t        valid_on_chip;          /* Boolean indication if Port is Valid on PSoC6 chip */
 ...
 * the PSoC6 does allow read of registers to get the current value but the s/w ia maintaining a shadow copy.
 */
...
typedef struct
{
    ...
    platform_pin_config_t        config;                /* Pin Pull Up/Down congiuration */
 ...
 ******************************************************/
/*
 * Table for all valid gpio pins maintained Per PSoC6 chip describing which IP Pins are brought out
 * in chip package and can be used by users. The expectation is that more tables will be compiled with
 * ifdef macros when newer PSoC6 based CPUs are supported.
 */
...
platform_result_t platform_gpio_deinit( const platform_gpio_t* gpio )
{
    ...
    uint32_t init_outval = 0;
    uint32_t init_hsiom = 0;      /* Should be default hsiom value for generic GPIO, which is 0 for PSoC6 */
