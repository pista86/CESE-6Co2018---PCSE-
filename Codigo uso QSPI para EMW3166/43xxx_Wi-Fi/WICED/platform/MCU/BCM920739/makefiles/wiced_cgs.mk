...
#	$(QUIET)$(eval UART:=$(shell $(CAT) < com_port.txt))
	$(QUIET)$(if $(UART), \
	        $(ECHO) Downloading application... && $(call CONV_SLASHES,$(CHIPLOAD_FULL_NAME)) -BLUETOOLMODE -PORT $(UART) -BAUDRATE $(PLATFORM_BAUDRATE) $(CHIPLOAD_REBAUD) $(CHIPLOAD_NO_READ_VERIFY) $(CHIPLOAD_CRC_VERIFY) -MINIDRIVER $(SOURCE_ROOT)platforms/$(PLATFORM)/$(PLATFORM_MINIDRIVER) -BTP $(SOURCE_ROOT)platforms/$(PLATFORM)/$(PLATFORM_BOOTP) -CONFIG $< -LOGTO build/$(OUTPUT_NAME)/logto.log > build/$(OUTPUT_NAME)/download.log 2>&1 \
